...
#  - Metric: derived from settings.rrd.data_sources[].name but with 'stdev' shown as 'varies'
#  - Time Range: from settings.html.time_ranges[].label  (fallback to legacy graph_time_ranges)
#
...
# Map internal DS name -> UI metric key
# Must match what rrd_exporter emits in JSON.
JSON_KEY_ALIAS = {
    "stdev": "varies",   # present to users as "Varies (ms)"
}

# Pretty labels for the metric selector
METRIC_LABELS = {
    "avg": "Avg (ms)",
    "last": "Last (ms)",
    "best": "Best (ms)",
    "loss": "Loss (%)",
    "varies": "Varies (ms)",  # StDev from MTR
}

def generate_target_html(ip, description, hops, settings, logger=None):
    # New logger init: directory/filename resolved from YAML
    ...
    # Time ranges (NEW helper; falls back to legacy graph_time_ranges)
    TIME_RANGES = [r for r in (get_html_ranges(settings) or []) if r.get("label")]

    # Metrics list from DS names, but convert 'stdev' -> 'varies' to match JSON/UI
    schema_metrics = [ds["name"] for ds in settings.get("rrd", {}).get("data_sources", []) if ds.get("name")]
    METRICS = [JSON_KEY_ALIAS.get(m, m) for m in schema_metrics]
...
<div class="wrap">
  ...
  <table><tr><th>Hop</th><th>Address</th><th>Details</th></tr>""")
            ...
            # render traceroute lines safely
...
<script>
// Metric keys available (already alias-mapped, e.g., 'stdev' -> 'varies'):
...
// Display labels for metric selector
...
function buildDatasetsFromBundle(bundle, metric) {
  return (bundle.hops || []).map(h => ({
    ...
    pointRadius: 2,
    tension: 0.25,
    borderDash: metric === 'varies' ? [5, 5] : undefined, // dashed for "varies"
...
def _labels_json(metric_keys):
    # emit a small JSON dict { key: label }
    pairs = []
...
def _labels_dict_js():
    # produces a JS object literal with all known labels
    parts = []
