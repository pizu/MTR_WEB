...
# ---------------- YAML-driven config helper ----------------
def _label_cfg(settings: dict) -> dict:
    """
    Pull label tuning knobs from settings['labels'], falling back to defaults.
    This is read every loop so YAML edits can take effect without code changes.
    """
    labels = settings.get("labels") or {}
    return {
        "reset_mode":         (labels.get("reset_mode") or "from_first_diff").strip().lower(),
...
def _apply_reset_policy(stats, prev_hops, curr_hops, reset_mode: str, logger=None):
    """
...
def monitor_target(ip, source_ip, settings, logger):
    ...
    while True:
        ...
        cfg = _label_cfg(settings)
        unstable_threshold = cfg["unstable_threshold"]
        topk_to_show       = cfg["topk_to_show"]
        majority_window    = cfg["majority_window"]
        sticky_min_wins    = cfg["sticky_min_wins"]
