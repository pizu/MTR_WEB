...
# Default: 2 x RRD step (in seconds); override with `graph_cleanup_safety_seconds` in YAML.
rrd_step = 60
try:
...
def cleanup_dir(path, days, extensions=None, label=None, min_age_seconds=0):
    ...
    for root, dirs, files in os.walk(path):
        for f in files:
            ...
            if safety_cutoff is not None and mtime >= safety_cutoff:
                # Too recent - keep it
                continue
    ...
    logger.info(
        f"[{label}] Scanned {scanned} file(s). Deleted {deleted} older than {days} day(s)."
        + (f" Safety buffer: {min_age_seconds}s." if min_age_seconds else "")
    )
